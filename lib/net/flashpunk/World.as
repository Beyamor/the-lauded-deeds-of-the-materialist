package net.flashpunk
{
	...
	public class World extends Tweener
	{
		...
		public function render():void 
		{
			...
			var e:Entity,
				i:int = _levelList.length;
			while (i --)
			{
				e = _renderLast[_levelList[i]];
				while (e)
				{
		 ...
		 */
		public function addGraphic(graphic:Graphic, level:int = 0, x:int = 0, y:int = 0):Entity
		{
		...
		/**
		 * Brings the Entity to the front of its contained level.
		...
		public function bringToFront(e:Entity):Boolean
		{
			...
			else _renderLast[e._level] = e._renderPrev;
			// place at the start
			e._renderNext = _renderFirst[e._level];
			e._renderNext._renderPrev = e;
			_renderFirst[e._level] = e;
			e._renderPrev = null;
			return true;
		}
		
		/**
		 * Sends the Entity to the back of its contained level.
		 * @param	e		The Entity to shift.
		 * @return	If the Entity changed position.
		 */
		public function sendToBack(e:Entity):Boolean
		{
			...
			else _renderFirst[e._level] = e._renderNext;
			// place at the end
			e._renderPrev = _renderLast[e._level];
			e._renderPrev._renderNext = e;
			_renderLast[e._level] = e;
			e._renderNext = null;
			return true;
		}
		
		/**
		 * Shifts the Entity one place towards the front of its contained level.
		 * @param	e		The Entity to shift.
		 * @return	If the Entity changed position.
		 */
		public function bringForward(e:Entity):Boolean
		{
			...
			else _renderFirst[e._level] = e;
			return true;
		}
		
		/**
		 * Shifts the Entity one place towards the back of its contained level.
		 * @param	e		The Entity to shift.
		 * @return	If the Entity changed position.
		 */
		public function sendBackward(e:Entity):Boolean
		{
			...
			else _renderLast[e._level] = e;
			return true;
		}
		
		/**
		 * If the Entity as at the front of its level.
		...
		/**
		 * If the Entity as at the back of its level.
		 * @param	e		The Entity to check.
		 * @return	True or false.
		 */
		...
		/**
		 * Returns the amount of Entities are on the level in the World.
		 * @param	level		The level to count Entities on.
		 * @return	How many Entities are on the level.
		 */
		public function levelCount(level:int):uint
		{
			return _levelCount[level] as uint;
		}
		...
		/**
		 * How many Entity levels the World has.
		 */
		public function get levels():uint { return _levelList.length; }
		...
		/**
		 * The first Entity on the Level.
		 * @param	level		The level to check.
		 * @return	The Entity.
		 */
		public function levelFirst(level:int):Entity
		{
			if (!_updateFirst) return null;
			return _renderFirst[level] as Entity;
		}
		
		/**
		 * The last Entity on the Level.
		 * @param	level		The level to check.
		 * @return	The Entity.
		 */
		public function levelLast(level:int):Entity
		{
			if (!_updateFirst) return null;
			return _renderLast[level] as Entity;
		}
		...
		{
			if (!_updateFirst) return null;
			return _renderLast[_levelList[_levelList.length - 1] as int] as Entity;
		}
		...
		{
			if (!_updateFirst) return null;
			return _renderFirst[_levelList[0] as int] as Entity;
		}
		
		/**
		 * The level that will be rendered first by the World.
		 */
		public function get levelFarthest():int
		{
			if (!_updateFirst) return 0;
			return _levelList[_levelList.length - 1] as int;
		}
		
		/**
		 * The level that will be rendered last by the World.
		 */
		public function get levelNearest():int
		{
			if (!_updateFirst) return 0;
			return _levelList[0] as int;
		}
		...
		/**
		 * Pushes all Entities in the World on the level into the Array or Vector.
		 * @param	level		The level to check.
		 * @param	into		The Array or Vector to populate.
		 * @return	The same array, populated.
		 */
		public function getLevel(level:int, into:Object):void
		{
			if (into is Array || into is Vector.<*>)
			{
				var e:Entity = _renderLast[level],
					n:uint = into.length;
				while (e)
				{
					into[n ++] = e;
		...
		public function updateLists():void
		{
			...
			if (_levelSort)
			{
				if (_levelList.length > 1) FP.sort(_levelList, true);
				_levelSort = false;
			}
		}
		...
		{
			var f:Entity = _renderFirst[e._level];
			if (f)
			{
				// Append entity to existing level.
				e._renderNext = f;
				f._renderPrev = e;
				_levelCount[e._level] ++;
			}
			else
			{
				// Create new level with entity.
				_renderLast[e._level] = e;
				_levelList[_levelList.length] = e._level;
				_levelSort = true;
				e._renderNext = null;
				_levelCount[e._level] = 1;
			}
			_renderFirst[e._level] = e;
			e._renderPrev = null;
		}
		...
		internal function removeRender(e:Entity):void
		{
			...
			else
			{
				...
				_renderFirst[e._level] = e._renderNext
				if (!e._renderNext)
				{
					// Remove the level from the level list if this was the last entity.
					if (_levelList.length > 1)
					{
						_levelList[_levelList.indexOf(e._level)] = _levelList[_levelList.length - 1];
						_levelSort = true;
					}
					_levelList.length --;
				}
